...
func UpdateAvatarHandler(store economy.PlayerStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Извлекаем username из контекста запроса (записывается JWT-middleware)
		...
		// Сохраняем изменения в файле через Store
...
func GetProfileHandler(store economy.PlayerStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// нам нужен баланс ДО вызова GetProfile для сравнения
		oldProfile := store.GetProfile(username) // Внутри сработает автоначисление

		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(oldProfile)
	}
}*/
