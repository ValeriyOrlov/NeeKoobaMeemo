...
func (h *AuthHandler) Register(c *fiber.Ctx) error {
	...
	ctx := c.Context()
	if err := h.authService.Register(ctx, req.Email, req.Username, req.Password); err != nil {
		status, msg := mapServiceError(err)
		return c.Status(status).JSON(fiber.Map{"error": msg})
	}
	return c.Status(fiber.StatusCreated).JSON(fiber.Map{"message": "verification email sent"})
}
...
func (h *AuthHandler) VerifyEmail(c *fiber.Ctx) error {
	...
	accessToken, refreshToken, user, err := h.authService.VerifyEmail(c.Context(), token)
	if err != nil {
